...
``GEOVAX_SAM_CHECKPOINT``. If either is missing, or inference raises, this silently and
...
class SAMFeatureExtractor:
    ...
    def __init__(self, model_type: str = "vit_h", checkpoint_path: Optional[str] = None):
        ...
        self.checkpoint_path = checkpoint_path or os.environ.get("GEOVAX_SAM_CHECKPOINT")
        self._device = "cpu"
    ...
    def _try_init_sam(self) -> bool:
        ...
        if not self.checkpoint_path:
            logger.info("No GEOVAX_SAM_CHECKPOINT configured; using classical DSM extraction.")
            return False
        try:
            import torch
        ...
        if not os.path.exists(self.checkpoint_path):
            logger.warning(
                "GEOVAX_SAM_CHECKPOINT=%s does not exist; using classical DSM extraction.",
