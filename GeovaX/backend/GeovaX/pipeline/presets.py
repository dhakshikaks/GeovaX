...
def default_layers(data_dir: str, max_features: int | None = None) -> list[LayerSpec]:
    """The Chennai catalogue, excluding optional sources that are not present locally.

    Some real data sources (for example the OSM ground-truth layer) are intentionally
    optional and may be unavailable in a constrained deployment. They should be skipped
    rather than causing the entire pipeline to fail.
    """
    layers = [
        LayerSpec(
