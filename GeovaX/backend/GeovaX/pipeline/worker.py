...
``python -m GeovaX.pipeline.worker``, but the module didn't exist — the container would
fail to start. This is the real implementation: a Redis-backed job consumer that pops run
requests pushed to the ``geovax:pipeline:jobs`` list and runs the actual, already-real
...
Push a job with e.g.::
    ...
    redis-cli LPUSH geovax:pipeline:jobs '{"aoi": "core", "data_dir": "data/aoi", "out_dir": "out/chennai"}'

Run standalone for local testing::

    python -m GeovaX.pipeline.worker --once '{"aoi": "test", "data_dir": "data/aoi", "out_dir": "out/tile"}'
"""
...
QUEUE_KEY = "geovax:pipeline:jobs"
