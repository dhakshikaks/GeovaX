...
from GeovaX.core.ids import (AdminContext, UlpinMinter, format_ulpin, geohash_encode,
                               mint_ulpin, validate_ulpin)
from GeovaX.core.ledger import ProvenanceLedger
from GeovaX.crs.engine import (AREA_UNITS_M2, CrsEngine, convert_area, format_extent,
                                 geodesic_area_m2, haversine_m, vincenty_m)
from GeovaX.pipeline.presets import default_layers
...
def test_default_layers_skip_missing_optional_sources(tmp_path):
    missing = tmp_path / "missing"
    missing.mkdir()
    layers = default_layers(str(missing))
    assert all(layer.dataset_id != "OSM_BUILDINGS_GT" for layer in layers)
    assert {layer.dataset_id for layer in layers} <= {"TNGIS_CADASTRE", "NCSCM_CADASTRE",
                                                     "GCC_BUILDINGS", "GOOGLE_OPEN_BUILDINGS",
                                                     "MS_BUILDINGS_TN"}
