...
def get_spark_session(app_name="GeovaXSedona"):
...
def harmonise_district(district_lgd: str):
    ...
    # 1. Load Data
    # cadastre_df = sedona.read.format("geoparquet").load(f"s3://geovax-datalake/raw/cadastre/{district_lgd}.parquet")
    # buildings_df = sedona.read.format("geoparquet").load(f"s3://geovax-datalake/raw/gobi/{district_lgd}.parquet")
    ...
    # 3. Write Output
    # joined_df.write.format("geoparquet").mode("overwrite").save(f"s3://geovax-datalake/harmonised/{district_lgd}.parquet")
    print(f"[Spark] Completed district {district_lgd}")


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="GeovaX Sedona Harmonisation Job")
