/**
 * GeovaX — SIH 2026 PS 26013 pitch deck.
 *
...
pres.author = "Team GeovaX";
pres.company = "Smart India Hackathon 2026";
pres.title = "GeovaX — PS 26013";
...
{
  const s = darkSlide(
    "GeovaX is a digital geospatial harmonisation platform for land records. Problem Statement 26013 from the Department " +
...
// ================================================================ 3 · what it emits
{
  const s = lightSlide("What GeovaX produces from that mess", "the solution",
  ...
  const items = [
    ...
    ["6", "OGC API - Features", "Another department consumes it from QGIS with no GeovaX-specific code."],
  ];
  items.forEach((it, i) => {
...
// ================================================================ 16 · close
{
  ...
  s.addText("GeovaX  ·  PS 26013  ·  Department of Land Resources  ·  SIH 2026", {
    x: M, y: 6.56, w: 12.1, h: 0.34, isTextBox: true, margin: 0, valign: "top",
    fontFace: B, fontSize: 11.5, color: LIGHTINK, charSpacing: 1,
  });
}

pres.writeFile({ fileName: process.argv[2] || "GeovaX_PS26013.pptx" })
